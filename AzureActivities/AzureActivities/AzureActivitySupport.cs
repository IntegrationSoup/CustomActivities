...
namespace AzureActivities
{
    ...
    internal static class AzureActivitySupport
    {
        ...
        internal static BlobUploadResponse Upload(BlobUploadRequest request, byte[] inputBytes)
        {
            ...
            string tempRoot = Path.Combine(Path.GetTempPath(), "AzureActivities", Guid.NewGuid().ToString("N"));
            ...
            try
            {
                ...
                if (!result.Success)
                {
                    throw new InvalidOperationException(
                        "The Azure runner process failed. " + FormatRunnerFailure(runnerExecutablePath, result));
                }

                if (!File.Exists(responsePath))
                {
                    throw new InvalidOperationException(
                        $"The Azure runner completed without creating '{responsePath}'.");
                }

                BlobUploadResponse response = DeserializeJson<BlobUploadResponse>(responsePath);
                if (!response.Success)
                {
                    throw new InvalidOperationException(response.Message ?? "The Azure runner reported a failure.");
                }
        ...
        private static string ResolveRunnerExecutablePath()
        {
            ...
            string activityDirectory = Path.GetDirectoryName(typeof(BlobSender).Assembly.Location) ?? string.Empty;
            ...
            throw new FileNotFoundException(
                $"The Azure runner executable could not be found. Place '{RunnerExecutableName}' under a '{RunnerDirectoryName}' folder beside the activity DLL, or set {RunnerPathEnvironmentVariable}.");
        }
