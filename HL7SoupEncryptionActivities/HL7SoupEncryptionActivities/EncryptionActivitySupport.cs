...
namespace HL7SoupEncryptionActivities
{
    ...
    internal static class EncryptionActivitySupport
    {
        ...
        internal static string Execute(EncryptionRequest request, string inputText)
        {
            ...
            string tempRoot = Path.Combine(Path.GetTempPath(), "EncryptionActivities", Guid.NewGuid().ToString("N"));
            string requestPath = Path.Combine(tempRoot, "request.json");
            string responsePath = Path.Combine(tempRoot, "response.json");
            string inputPath = Path.Combine(tempRoot, "input.txt");
            string outputPath = Path.Combine(tempRoot, "output.txt");
            ...
            try
            {
                ...
                File.WriteAllText(inputPath, inputText ?? string.Empty, new UTF8Encoding(encoderShouldEmitUTF8Identifier: false));
                ...
                if (!result.Success)
                {
                    throw new InvalidOperationException(
                        "The encryption runner process failed. " + FormatRunnerFailure(runnerExecutablePath, result));
                }

                if (!File.Exists(responsePath))
                {
                    throw new InvalidOperationException(
                        $"The encryption runner completed without creating '{responsePath}'.");
                }

                EncryptionResponse response = DeserializeJson<EncryptionResponse>(responsePath);
                if (!response.Success)
                {
                    throw new InvalidOperationException(response.Message ?? "The encryption runner reported a failure.");
                }

                if (!File.Exists(outputPath))
                {
                    throw new InvalidOperationException(
                        $"The encryption runner reported success but did not create '{outputPath}'.");
                }
        ...
        private static string ResolveRunnerExecutablePath()
        {
            ...
            string activityDirectory = Path.GetDirectoryName(typeof(EncryptMessage).Assembly.Location) ?? string.Empty;
            ...
            throw new FileNotFoundException(
                $"The encryption runner executable could not be found. Place '{RunnerExecutableName}' under a '{RunnerDirectoryName}' folder beside the activity DLL, or set {RunnerPathEnvironmentVariable}.");
        }
